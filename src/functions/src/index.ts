...
export const getVendorDataset = onCall({ region: "us-central1" }, async (request: CallableRequest) => {
    try {
      ...
      const contentString = contents.toString("utf8");

      // Fix: Check if the content is empty before parsing
      if (!contentString) {
          logger.warn(`getVendorDataset: Snapshot file at '${filePath}' is empty.`);
          return {
              version: 'empty',
              status: 'modified',
              data: {
      ...
      const dataset = JSON.parse(contentString);
