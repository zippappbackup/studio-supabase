...
exports.manuallyRunZippHighlights = exports.placePhotoProxy = exports.scheduledVendorSnapshot = exports.onVendorDelete = exports.onVendorCreate = exports.updateZippHighlights = exports.manuallyGenerateVendorSnapshot = exports.getVendorDataset = exports.finalizeVendorClaim = exports.migrateVendorPhotos = exports.uploadImage = void 0;
// FORCE DEPLOY 25
...
const google_places_api_1 = require("./google-places-api");
...
exports.uploadImage = (0, https_1.onCall)({ region: "us-central1" }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', "Authentication required to upload files.");
    }
    const { file, path } = request.data;
    if (!file || !path) {
        throw new https_1.HttpsError('invalid-argument', 'The function must be called with "file" and "path" arguments.');
    }
    try {
        const mimeType = file.match(/data:(.*);base64,/)?.[1];
        if (!mimeType) {
            throw new https_1.HttpsError('invalid-argument', 'Invalid data URI format.');
        }
        ...
        throw new https_1.HttpsError('internal', `Image upload failed: ${error.message}`);
    }
});
exports.migrateVendorPhotos = (0, https_1.onRequest)({ region: "us-central1", timeoutSeconds: 540, secrets: ["PLACES_KEY"], cors: true }, async (req, res) => {
    ...
    try {
        ...
        const logs = [`Starting batch migration for up to ${limit} vendors.`];
        ...
        const vendorsToProcess = vendorDocs.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        ...
        for (const vendor of vendorsToProcess) {
            ...
            const placeDetails = await (0, google_places_api_1.fetchPlaceDetails)(vendor.googlePlaceId, apiKey);
            ...
            for (const photoUrl of placeDetails.photos) {
                ...
                try {
                    const gcpResponse = await (0, node_fetch_1.default)(photoUrl);
...
exports.finalizeVendorClaim = (0, https_1.onCall)({ region: "us-central1" }, async (request) => {
    const { vendorId } = request.data;
    const uid = request.auth?.uid;
    if (!uid) {
        throw new https_1.HttpsError("unauthenticated", "You must be logged in to claim a vendor.");
    }
    if (!vendorId) {
        throw new https_1.HttpsError("invalid-argument", "The function must be called with a 'vendorId'.");
    }
    ...
    try {
        await db.runTransaction(async (transaction) => {
            ...
            if (!vendorDoc.exists) {
                return Promise.reject(new https_1.HttpsError("not-found", `Vendor ${vendorId} not found.`));
            }
            const vendorData = vendorDoc.data();
            const alreadyClaimed = vendorData?.claimedBy != null && vendorData.claimedBy !== "";
            if (alreadyClaimed) {
                return Promise.reject(new https_1.HttpsError("failed-precondition", "This business has already been claimed."));
            }
        ...
        if (error.code && error.message && error instanceof https_1.HttpsError === false) {
            throw error;
        }
        throw new https_1.HttpsError("internal", "An unexpected error occurred while finalizing the claim.");
    }
});
exports.getVendorDataset = (0, https_1.onCall)({ region: "us-central1" }, async (request) => {
    try {
        const configRef = db.collection("config").doc("vendorDataset");
        const configSnap = await configRef.get();
        if (!configSnap.exists) {
            throw new https_1.HttpsError("not-found", "Configuration for the vendor dataset could not be found.");
        }
        const { filePath, version } = configSnap.data();
        if (!filePath || !version) {
            throw new https_1.HttpsError("invalid-argument", "Dataset configuration is incomplete. Missing 'filePath' or 'version'.");
        }
        ...
        throw new https_1.HttpsError("internal", err.message, err);
    }
});
exports.manuallyGenerateVendorSnapshot = (0, https_1.onCall)({ region: "us-central1", timeoutSeconds: 300, cors: true }, async (request) => {
...
exports.updateZippHighlights = (0, scheduler_1.onSchedule)({ region: "us-central1", schedule: 'every 24 hours' }, async (event) => {
    ...
    try {
        ...
        for (const categoryId of categories) {
            const querySnapshot = await vendorsRef
                ...
                .get();
            const vendorsInCategory = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
...
exports.onVendorCreate = (0, firestore_1.onDocumentCreated)({ region: "us-central1", document: 'vendors/{vendorId}' }, async (event) => {
...
exports.onVendorDelete = (0, firestore_1.onDocumentDeleted)({ region: "us-central1", document: 'vendors/{vendorId}' }, async (event) => {
    const statsRef = db.collection('analytics').doc('stats');
    try {
        await statsRef.set({
            totalVendors: admin.firestore.FieldValue.increment(-1)
...
exports.scheduledVendorSnapshot = (0, scheduler_1.onSchedule)({ region: "us-central1", schedule: 'every 4 hours' }, async (event) => {
...
exports.placePhotoProxy = (0, https_1.onRequest)({ secrets: ["PLACES_KEY"], cors: true, region: 'us-central1' }, async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    if (req.method === 'OPTIONS') {
    ...
    try {
        ...
        const response = await (0, node_fetch_1.default)(photoUrl);
...
exports.manuallyRunZippHighlights = (0, https_1.onRequest)({ region: "us-central1" }, (req, res) => {
