...
import { Client, Place, PlaceDetailsResponse, TextSearchRequest, PlaceDetailsRequest, PlacePhoto } from "@googlemaps/google-maps-services-js";
...
const categoryQueryMap: { [key: string]: { types: string[], keywords: string[] } } = {
  'car-cleaning': {
 ...
 */
export async function fetchPlaceDetails(placeId: string, apiKey: string): Promise<any | null> {
    ...
    try {
        const response: PlaceDetailsResponse = await client.placeDetails({ params: detailsParams });
        ...
        const photoUrls = (placeDetails.photos || []).map((photo: PlacePhoto) => 
...
export async function searchPlacesByKeyword(
    ...
    search_loop:
    for (const keyword of queryConfig.keywords) {
        for (const type of queryConfig.types) {
            let nextPageToken: string | null = null;
            let pageCount = 0;

            do {
                if (isLimitEnabled && allResults.size >= vendorLimit) break search_loop;
                ...
                const searchParams: TextSearchRequest['params'] = {
                    ...
                    type: type as any, // Cast to any to satisfy the SDK's stricter enum typing
                    key: apiKey,
                ...
                try {
                    ...
                    if (data.results) {
                        for (const result of data.results) {
                            if (!result.formatted_address?.toLowerCase().includes('singapore')) continue;
                            if (isLimitEnabled && allResults.size >= vendorLimit) break search_loop;
